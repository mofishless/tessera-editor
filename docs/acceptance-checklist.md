...
> 版本标注：**M1**（无 AI 的编辑器地板）、**M2**（AI 骨架）、**v1.1**（表格/历史/评论）、**v1.x**（预留位）。
...
| 链接 | ⌘K | Ctrl K | ✅（链接面板） |
| 占位符 | ⌘⌥P | Ctrl Alt P | ✅（toggle 占位符 mark） |
...
- [x] **持久化版本历史**：StorageService 注入 ✅；空闲自动快照（idleMs 可配）+ 手动捕获 ✅；面板块级+词级 diff（红绿）✅（实测改动标题 → diff 显示 add/del 词级）；一键恢复（单事务可撤销）✅（实测恢复原标题）
...
- [x] **占位符**：⌘⌥P toggle + 斜杠 /待填人 /待填日期 token（data-kind）✅
